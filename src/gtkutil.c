...
# ifdef HAVE_GTK3
static void emacs_menu_bar_get_preferred_width (GtkWidget *, gint *,
						gint *);
...
static bool xg_widget_key_press_event_cb (GtkWidget *, GdkEvent *,
					  gpointer);
# endif
...
#  define gtk_scrollbar_new(ori, spacing) \
    ((ori) == GTK_ORIENTATION_HORIZONTAL  \
       ? gtk_hscrollbar_new (spacing)     \
       : gtk_vscrollbar_new (spacing))
# endif /* HAVE_GTK3 */
...
static void
emacs_menu_bar_get_preferred_width (GtkWidget *widget, gint *minimum,
				    gint *natural)
{
  GtkWidgetClass *widget_class;
...
# else
  if (FRAME_X_DISPLAY (f) != DEFAULT_GDK_DISPLAY ())
    {
      GdkScreen *gscreen
	= gdk_display_get_default_screen (FRAME_X_DISPLAY (f));
...
/* Open a display named by DISPLAY_NAME.  The display is returned in
   *DPY. *DPY is set to NULL if the display can't be opened.

   Returns non-zero if display could be opened, zero if display could
   not be opened, and less than zero if the GTK version doesn't
...
# else
  gdpy = gdk_display_open (strlen (display_name) == 0 ? NULL
						      : display_name);
# endif
  request_sigio ();
  if (!gdpy_def && gdpy)
	...
	if (dpyinfo->display != dpy)
	  {
	    ...
	    gdk_display_manager_set_default_display (
	      gdk_display_manager_get (), gdpy_new);
	    break;
	  }
      gdpy_def = gdpy_new;
    }
...
static GdkPixbuf *
xg_get_pixbuf_from_pix_and_mask (struct frame *f, Pixmap pix,
				 Pixmap mask)
{
  ...
  if (FRAME_DISPLAY_INFO (f)->red_bits != 8)
    return 0;
  XGetGeometry (FRAME_X_DISPLAY (f), pix, &wunused, &iunused,
		&iunused, &width, &height, &uunused, &depth);
  if (depth != 24)
    return 0;
  XImage *xim = XGetImage (FRAME_X_DISPLAY (f), pix, 0, 0, width,
			   height, ~0, XYPixmap);
  if (xim)
    {
      XImage *xmm
	= (!mask ? 0
		 : XGetImage (FRAME_X_DISPLAY (f), mask, 0, 0, width,
			      height, ~0, XYPixmap));
      icon_buf
      ...
      if (icon_buf)
	{
	  ...
	  int rowjunkwidth
	    = gdk_pixbuf_get_rowstride (icon_buf) - width * 4;
...
/* For the image defined in IMG, make and return a GtkImage.  For
   displays with 8 planes or less we must make a GdkPixbuf and apply
   the mask manually. Otherwise the highlighting and dimming the tool
   bar code in GTK does will look bad.  For display with more than 8
   planes we just use the pixmap and mask directly.  For monochrome
   displays, GTK doesn't seem able to use external pixmaps, it looks
   bad whatever we do. The image is defined on the display where frame
   F is. WIDGET is used to find the GdkColormap to use for the
   GdkPixbuf. If OLD_WIDGET is NULL, a new widget is constructed and
   returned. If OLD_WIDGET is not NULL, that widget is modified.  */

static GtkWidget *
xg_get_image_for_pixmap (struct frame *f, struct image *img,
...
# endif
  ...
  /* If we have a file, let GTK do all the image handling.
     This seems to be the only way to make insensitive and activated
     icons look good in all cases.  */
  ...
  if (STRINGP (specified_file)
    ...
    /* No file, do the image handling ourselves.  This will look very
       bad on a monochrome display, and sometimes bad on all displays
       with certain themes.  */

# if defined(USE_CAIRO) || defined(USE_SKIA)
  if (cairo_pattern_get_type (img->cr_data)
...
# else /* !USE_CAIRO && !USE_SKIA */
  /* This is a workaround to make icons look good on pseudo color
     ...
     and is strictly not needed on true color/static color displays
     (i.e. 16 bits and higher).  But we do it anyway so we get a
     pixbuf that is not associated with the img->pixmap.  The
     img->pixmap may be removed by clearing the image cache and then
     the tool bar redraw fails, since Gtk+ assumes the pixmap is
     always there.  */
  icon_buf
    = xg_get_pixbuf_from_pix_and_mask (f, img->pixmap, img->mask);
...
xg_set_cursor (GtkWidget *w, GdkCursor *cursor)
{
  ...
  /* The scroll bar widget has more than one GDK window (had to look
     at the source to figure this out), and there is no way to set
     cursor on widgets in GTK.  So we must set the cursor for all GDK
     windows. Ditto for menus.  */
...
get_utf8_string (const char *str)
{
  ...
  if (!utf8_str)
	...
	g_free (cp);
	  ...
	  || ckd_add (&alloc, alloc, len + 1) || SIZE_MAX < alloc)
...
bool
xg_check_special_colors (struct frame *f, const char *color_name,
			 Emacs_Color *color)
{
  bool success_p;
  bool get_bg;
...
/* Gtk+ calls this callback when the parent of our tooltip dummy
   changes. We use that to pop down the tooltip.  This happens if Gtk+
   for some reason wants to change or hide the tooltip.  */

static void
hierarchy_ch_cb (GtkWidget *widget, GtkWidget *previous_toplevel,
		 gpointer user_data)
{
...
static gboolean
qttip_cb (GtkWidget *widget, gint xpos, gint ypos,
  ...
  if (x->ttip_widget == NULL)
    {
      ...
      x->ttip_window
	= GTK_WINDOW (gtk_widget_get_toplevel (x->ttip_lbl));
...
bool
xg_prepare_tooltip (struct frame *f, Lisp_Object string, int *width,
		    int *height)
{
  xp_output *x = f->output_data.xp;
  GtkWidget *widget;
  ...
  g_object_set_data (G_OBJECT (gtk_widget_get_display (
		       GTK_WIDGET (x->ttip_window))),
  ...
  gtk_widget_get_preferred_size (GTK_WIDGET (x->ttip_window), NULL,
				 &req);
  if (width)
    *width = req.width;
  if (height)
...
void
xg_show_tooltip (struct frame *f, Lisp_Object string)
{
...

/***********************************************************************
	General functions for creating widgets, resizing, events,
    e.t.c.
     ***********************************************************************/
...
# if !GTK_CHECK_VERSION(3, 22, 0)
      ...
      else
	{
	  ...
	  sprintf (geom_str, "=%dx%d%c%d%c%d", FRAME_PIXEL_WIDTH (f),
		   FRAME_PIXEL_HEIGHT (f), (xneg ? '-' : '+'), left,
		   (yneg ? '-' : '+'), top);

	  /* Silence warning about visible children.  */
	  id = g_log_set_handler ("Gtk",
				  G_LOG_LEVEL_WARNING
				    | G_LOG_FLAG_FATAL
				    | G_LOG_FLAG_RECURSION,
				  my_log_handler, NULL);

	  if (!gtk_window_parse_geometry (GTK_WINDOW (
...
xg_frame_resized (struct frame *f, int width, int height)
{
  ...
  if (width != FRAME_PIXEL_WIDTH (f)
	...
	frame_size_history_extra (f,
				  build_string (
				    "xg_frame_resized, changed"),
				  FRAME_PIXEL_WIDTH (f),
				  FRAME_PIXEL_HEIGHT (f), width,
				  height,
    ...
    frame_size_history_extra (f,
			      build_string (
				"xg_frame_resized, unchanged"),
			      FRAME_PIXEL_WIDTH (f),
			      FRAME_PIXEL_HEIGHT (f), width, height,
			      f->new_size_p ? f->new_width : -1,
			      f->new_size_p ? f->new_height : -1);
}
...
# endif
  ...
  /* Do this before resize, as we don't know yet if we will be
   * resized.  */
  FRAME_RIF (f)->clear_under_internal_border (f);
  ...
  /* Resize the top level widget so rows and columns remain constant.
     ...
     or it is fullwidth and we only want to change the height we
     should be able to preserve the fullscreen property.  However, due
     to the fact that we have to send a resize request anyway, the
     window manager will abolish it.  At least the respective size
     should remain unchanged but giving the frame back its normal size
     will be broken ... */
...
# else
	...
	gtk_window_resize (GTK_WINDOW (FRAME_GTK_OUTER_WIDGET (f)),
      ...
      {
	gtk_widget_set_size_request (FRAME_GTK_WIDGET (f), gwidth,
				     outer_height);
      }
# endif
  ...
  if (FRAME_VISIBLE_P (f) && !was_visible)
    {
      if (CONSP (frame_size_history))
	frame_size_history_extra (f,
				  build_string ("xg_frame_set_char_"
						"size, visible"),
				  FRAME_PIXEL_WIDTH (f),
				  FRAME_PIXEL_HEIGHT (f), width,
				  height, f->new_width,
...
# endif
	...
	frame_size_history_extra (f,
      ...
      adjust_frame_size (f, FRAME_PIXEL_TO_TEXT_WIDTH (f, width),
			 FRAME_PIXEL_TO_TEXT_HEIGHT (f, height), 5, 0,
...
void
xg_frame_set_size_and_position (struct frame *f, int width,
				int height)
{
  int outer_height
    = height + FRAME_TOOLBAR_HEIGHT (f) + FRAME_MENUBAR_HEIGHT (f);
  ...
  /* Full force ahead.  For top-level frames the gravity will get
     reset to NorthWestGravity anyway.  */
  flags |= USSize;
...
# else
  if (FRAME_GTK_OUTER_WIDGET (f))
    ...
    gtk_widget_set_size_request (FRAME_GTK_WIDGET (f), outer_width,
				 outer_height);
# endif
...
# ifndef HAVE_PGTK
/* Convert an X Window WSESC on display DPY to its corresponding
   GtkWidget. Must be done like this, because GtkWidget:s can have
   "hidden" X Window that aren't accessible.
...
#  endif
	...
	= "* { background-color: #%02x%02x%02x; }";
      ...
      sprintf (buffer, format, xbg.red >> 8, xbg.green >> 8,
	       xbg.blue >> 8);
...
/* Callback called when the gtk theme changes.
   We notify lisp code so it can fix faces used for region for
   example.  */

static void
style_changed_cb (GObject *go, GParamSpec *spec, gpointer user_data)
{
...
# endif
  ...
  /* If scroll bar width changed, we need set the new size on all
     frames on this display.  */
  if (dpy)
    {
...
# endif
	    && FRAME_X_DISPLAY (f) == dpy)
	  {
	    ...
	    FRAME_TERMINAL (f)->set_scroll_bar_default_height_hook (
	      f);
...
# ifndef HAVE_PGTK
static gboolean
delete_cb (GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
  return TRUE;
}
# endif
...
# endif
  ...
  /* gtk_window_set_has_resize_grip is a Gtk+ 3.0 function but Ubuntu
     ...
     Gtk+ 2.0 applications also.  But it has a bug that makes Emacs
     loop forever, so disable the grip.  */
...
# endif
  ...
  /* Use same names as the Xt port does.  I.e. Emacs.pane.emacs by
   * default */
  ...
  /* We don't want this widget double buffered, because we draw on it
     ...
     will make the widget blank, and then Emacs redraws it.  This
     flickers a lot, so we turn off double buffering.  */
...
# endif
  ...
  xg_set_geometry (f);
  f->win_gravity = gtk_window_get_gravity (
    GTK_WINDOW (FRAME_GTK_OUTER_WIDGET (f)));

  gtk_widget_add_events (wfixed, GDK_POINTER_MOTION_MASK
# ifndef HAVE_PGTK
				   | GDK_EXPOSURE_MASK
# endif
...
# if !defined HAVE_PGTK
  GdkVisual *visual
    = gdk_x11_screen_lookup_visual (screen, XVisualIDFromVisual (
					      FRAME_X_VISUAL (f)));
...
# ifndef HAVE_PGTK
  /* Must realize the windows so the X window gets created.  It is
     used by callers of this function.  */
...
# ifndef HAVE_PGTK
  ...
  g_signal_connect (G_OBJECT (wfixed), "key-press-event",
		    G_CALLBACK (xg_widget_key_press_event_cb), NULL);
# endif

  {
    GtkSettings *gs = gtk_settings_get_for_screen (screen);
    /* Only connect this signal once per screen.  */
    if (!g_signal_handler_find (G_OBJECT (gs), G_SIGNAL_MATCH_FUNC, 0,
				0, 0,
				(gpointer) G_CALLBACK (
				  style_changed_cb),
				0))
...
void
xg_wm_set_size_hint (struct frame *f, long int flags,
		     bool user_position)
{
...
# ifndef HAVE_PGTK
      && (x_wm_supports (f,
			 FRAME_DISPLAY_INFO (f)->Xatom_net_wm_state)
	  || x_wm_supports (f, FRAME_DISPLAY_INFO (f)
				 ->Xatom_net_wm_state_fullscreen))
# endif
  )
    {
      /* Don't set hints when maximized or fullscreen.  Apparently
	 KWin and Gtk3 don't get along and the frame shrinks (!).
      */
      return;
    }
  ...
  base_width
    = FRAME_TEXT_COLS_TO_PIXEL_WIDTH (f, 1) + FRAME_TOOLBAR_WIDTH (f);
  ...
  /* These currently have a one to one mapping with the X values, but
  ...
  if (hint_flags != f->output_data.xp->hint_flags
      || memcmp (&size_hints, &f->output_data.xp->size_hints,
		 sizeof (size_hints))
	   != 0)
    {
      block_input ();
...
xg_set_background_color (struct frame *f, unsigned long bg)
{
  if (FRAME_GTK_WIDGET (f))
    {
      block_input ();
      xg_set_widget_bg (f, FRAME_GTK_WIDGET (f),
...
# ifndef HAVE_PGTK
  if (FRAME_GTK_WIDGET (f))
    gdk_window_set_skip_taskbar_hint (gtk_widget_get_window (
					FRAME_GTK_OUTER_WIDGET (f)),
				      NILP (skip_taskbar) ? FALSE
							  : TRUE);
# else
  if (FRAME_GTK_OUTER_WIDGET (f))
    gdk_window_set_skip_taskbar_hint (gtk_widget_get_window (
  ...
  if (FRAME_GTK_WIDGET (f))
    {
      ...
      gboolean g_no_focus_on_map
	= NILP (no_focus_on_map) ? TRUE : FALSE;
...
# ifdef HAVE_PGTK
  if (!FRAME_GTK_OUTER_WIDGET (f))
    {
      if (FRAME_WIDGET (f))
	gtk_widget_set_can_focus (FRAME_WIDGET (f),
				  g_no_accept_focus);
      return;
    }
# endif
...
void
xg_set_override_redirect (struct frame *f,
			  Lisp_Object override_redirect)
{
  ...
  if (FRAME_GTK_OUTER_WIDGET (f))
	...
	= gtk_widget_get_window (FRAME_GTK_OUTER_WIDGET (f));
      ...
      gdk_window_set_override_redirect (gwin, NILP (override_redirect)
						? FALSE
						: TRUE);
    }
...
void
xg_set_frame_icon (struct frame *f, Pixmap icon_pixmap,
		   Pixmap icon_mask)
{
#  ifdef HAVE_PGTK
  if (!FRAME_GTK_OUTER_WIDGET (f))
...
#  endif
  GdkPixbuf *gp
    = xg_get_pixbuf_from_pix_and_mask (f, icon_pixmap, icon_mask);
  if (gp)
...
/* Callback for dialogs that get WM_DELETE_WINDOW.  We pop down
   ...
   in GTK.  This prevents GTK from destroying the dialog widget
   automatically and we can always destroy the widget manually,
   regardless of how it was popped down (button press or
   WM_DELETE_WINDOW). W is the dialog widget. EVENT is the GdkEvent
   that represents WM_DELETE_WINDOW (not used). user_data is NULL (not
   used).

   Returns TRUE to end propagation of event.  */

static gboolean
dialog_delete_callback (GtkWidget *w, GdkEvent *event,
			gpointer user_data)
{
  gtk_widget_unmap (w);
  return TRUE;
...
static GtkWidget *
create_dialog (widget_value *wv, GCallback select_cb,
	       GCallback deactivate_cb)
{
  ...
  /* If the number of buttons is greater than 4, make two rows of
     buttons instead.  This looks better.  */
  bool make_two_rows = total_buttons > 4;

# if GTK_CHECK_VERSION(3, 12, 0)
  GtkBuilder *gbld = gtk_builder_new ();
  GObject *go
    = gtk_buildable_get_internal_child (GTK_BUILDABLE (wd), gbld,
					"action_area");
...
# endif
  ...
  if (make_two_rows)
    {
      GtkWidget *wvbox
	= gtk_box_new (GTK_ORIENTATION_VERTICAL, button_spacing);
      GtkWidget *whbox_up
	= gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
      ...
      gtk_box_pack_start (GTK_BOX (wvbox), whbox_down, FALSE, FALSE,
			  0);
  ...
  if (deactivate_cb)
    {
      g_signal_connect (G_OBJECT (wdialog), "close", deactivate_cb,
			0);
      g_signal_connect (G_OBJECT (wdialog), "response", deactivate_cb,
			0);
    }
  ...
  for (item = wv->contents; item; item = item->next)
      ...
      if (item->name && strcmp (item->name, "message") == 0)
	{
	  ...
	  gtk_box_pack_start (wvbox, gtk_label_new (""), FALSE, FALSE,
			      0);
...
# endif
      ...
      else
	{
	  ...
	  if (select_cb)
	    g_signal_connect (G_OBJECT (w), "clicked", select_cb,
			      item->call_data);
...
static void
xg_dialog_response_cb (GtkDialog *w, gint response,
		       gpointer user_data)
{
...
/* If there are any emacs timers pending, add a timeout to main loop
   in DATA. Pass DATA as gpointer so we can use this as a callback. */
...
xg_maybe_add_timer (gpointer data)
{
  ...
  if (timespec_valid_p (next_time))
	...
	dd->timerid
	  = g_timeout_add (s * 1000 + ms, xg_maybe_add_timer, dd);
    }
  return FALSE;
}

/* Pops up a modal dialog W and waits for response.
   We don't use gtk_dialog_run because we want to process emacs
   timers. The dialog W is not destroyed when this function returns.
 */
...
xg_dialog_run (struct frame *f, GtkWidget *w)
{
  ...
  g_signal_connect (G_OBJECT (w), "response",
		    G_CALLBACK (xg_dialog_response_cb), &dd);
  /* Don't destroy the widget if closed by the window manager close
   * button.  */
...
static void
xg_toggle_notify_cb (GObject *gobject, GParamSpec *arg1,
		     gpointer user_data)
{
  if (strcmp (arg1->name, "show-hidden") == 0)
    {
      ...
      g_object_get (G_OBJECT (gobject), "show-hidden", &visible,
		    NULL);
      toggle_on
	...
	{
	  gpointer cb
	    = (gpointer) G_CALLBACK (xg_toggle_visibility_cb);
	  g_signal_handlers_block_by_func (G_OBJECT (wtoggle), cb,
					   gobject);
	  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (wtoggle),
					visible);
	  g_signal_handlers_unblock_by_func (G_OBJECT (wtoggle), cb,
					     gobject);
	}
...
/* Read a file name from the user using a file chooser dialog.
   ...
   DEFAULT_FILENAME is a default selection to be displayed.  May be
   NULL. If MUSTMATCH_P, the returned file name must be an existing
   file.  (Actually, this only has cosmetic effects, the user can
   still enter a non-existing file.)  *FUNC is set to a function that
   can be used to retrieve the selected file name from the returned
   widget.

   Returns the created widget.  */

static GtkWidget *
xg_get_file_with_chooser (struct frame *f, char *prompt,
			  char *default_filename, bool mustmatch_p,
			  bool only_dir_p, xg_get_file_func *func)
{
  char msgbuf[1024];
  ...
  GtkFileChooserAction action
    = (mustmatch_p ? GTK_FILE_CHOOSER_ACTION_OPEN
  ...
  if (x_gtk_file_dialog_help_text)
    {
      char *z = msgbuf;
      /* Gtk+ 2.10 has the file name text entry box integrated in the
	 dialog. Show the C-l help text only for versions < 2.10.  */
  ...
  gtk_file_chooser_set_extra_widget (GTK_FILE_CHOOSER (filewin),
				     wbox);
  ...
  if (default_filename)
    {
      ...
      /* File chooser does not understand ~/... in the file name.  It
	 must be an absolute name starting with /.  */
...
static GtkWidget *
xg_get_file_with_selection (struct frame *f, char *prompt,
			    char *default_filename, bool mustmatch_p,
			    bool only_dir_p, xg_get_file_func *func)
{
  GtkWidget *filewin;
...
/* Read a file name from the user using a file dialog, either the old
   file selection dialog, or the new file chooser dialog.  Which to
   use depends on what the GTK version used has, and what the value of
...
char *
xg_get_file_name (struct frame *f, char *prompt,
		  char *default_filename, bool mustmatch_p,
		  bool only_dir_p)
{
  GtkWidget *w = 0;
...
static Lisp_Object
xg_weight_to_symbol (PangoWeight w)
{
  return (w <= PANGO_WEIGHT_THIN	 ? Qthin	/* 100 */
...
static Lisp_Object
xg_style_to_symbol (PangoStyle s)
{
  return (s == PANGO_STYLE_OBLIQUE  ? Qoblique
...
#  ifdef HAVE_GTK3
      ...
      if (desc)
	{
	  ...
	  font = CALLN (Ffont_spec, QCfamily, build_string (family),
			QCsize,
...
/* The name of menu items that can be used for customization.  Since
   GTK RC files are very crude and primitive, we have to set this on
   all menu item names so a user can easily customize menu items.  */

# define MENU_ITEM_NAME "emacs-menuitem"

/* Linked list of all allocated struct xg_menu_cb_data.  Used for
   marking during GC.  The next member points to the items.  */
static xg_list_node xg_menu_cb_list;

/* Linked list of all allocated struct xg_menu_item_cb_data.  Used for
   marking during GC.  The next member points to the items.  */
static xg_list_node xg_menu_item_cb_list;

/* Allocate and initialize CL_DATA if NULL, otherwise increase
   ref_count. F is the frame CL_DATA will be initialized for.
   HIGHLIGHT_CB is the callback to call when entering/leaving menu
   items.
...
make_cl_data (xg_menu_cb_data *cl_data, struct frame *f,
...
/* Update CL_DATA with values from frame F and with HIGHLIGHT_CB.
   ...
   removed, so menu_bar_vector and menu_bar_items_used change.  We
   must then update CL_DATA since it is used to determine which menu
...
static void
update_cl_data (xg_menu_cb_data *cl_data, struct frame *f,
		GCallback highlight_cb)
{
  if (cl_data)
    {
...
/* Callback called when a menu item is destroyed.  Used to free data.
   ...
   CLIENT_DATA points to the xg_menu_item_cb_data associated with the
   W.  */
...
static gboolean
menuitem_highlight_callback (GtkWidget *w, GdkEventCrossing *event,
			     gpointer client_data)
{
  ...
  if (data)
	...
	{
	  gpointer call_data
	    = event->type == GDK_LEAVE_NOTIFY ? 0 : data;
	  GtkCallback func
	    = (GtkCallback) data->cl_data->highlight_cb;
...
static GtkWidget *
make_widget_for_menu_item (const char *utf8_label,
			   const char *utf8_key)
{
  GtkWidget *wlbl;
  GtkWidget *wkey;
...
/* Make and return a menu item widget with the key to the right.
   UTF8_LABEL is the text for the menu item (GTK uses UTF8
   internally). UTF8_KEY is the text representing the key binding.
   ITEM is the widget_value describing the menu item.

   GROUP is an in/out parameter.  If the menu item to be created is
   not part of any radio menu group, *GROUP contains NULL on entry and
   exit. If the menu item to be created is part of a radio menu group,
   on entry *GROUP contains the group to use, or NULL if this is the
   first item in the group.  On exit, *GROUP contains the radio item
   group.
...
static GtkWidget *
make_menu_item (const char *utf8_label, const char *utf8_key,
		widget_value *item, GSList **group)
{
  GtkWidget *w;
  GtkWidget *wtoadd = 0;

  /* It has been observed that some menu items have a NULL name field.
     This will lead to this function being called with a NULL
     utf8_label. GTK crashes on that so we set a blank label.  Why
     there is a NULL name remains to be investigated.  */
  if (!utf8_label)
    utf8_label = " ";
  ...
  if (item->button_type == BUTTON_TYPE_TOGGLE)
	...
	w = gtk_check_menu_item_new_with_label (utf8_label);
    ...
    {
      if (utf8_key)
	w = gtk_radio_menu_item_new (*group);
      else
	w = gtk_radio_menu_item_new_with_label (*group, utf8_label);
      *group
...
/* Create a menu item widget, and connect the callbacks.
   ...
   HIGHLIGHT_CB is the callback to call when entering/leaving menu
   items. CL_DATA points to the callback data to be used for this
   menu. GROUP is an in/out parameter.  If the menu item to be created
   is not part of any radio menu group, *GROUP contains NULL on entry
   and exit. If the menu item to be created is part of a radio menu
   group, on entry *GROUP contains the group to use, or NULL if this
   is the first item in the group.  On exit, *GROUP contains the radio
   item group.

   Returns the created GtkWidget.  */

static GtkWidget *
xg_create_one_menuitem (widget_value *item, struct frame *f,
			GCallback select_cb, GCallback highlight_cb,
			xg_menu_cb_data *cl_data, GSList **group)
{
  char *utf8_label;
  char *utf8_key;
  GtkWidget *w;
  ...
  g_signal_connect (G_OBJECT (w), "destroy",
		    G_CALLBACK (menuitem_destroy_callback), cb_data);
...
/* Create a full menu tree specified by DATA.
   F is the frame the created menu belongs to.
   SELECT_CB is the callback to use when a menu item is selected.
   DEACTIVATE_CB is the callback to use when a sub menu is not shown
   anymore. HIGHLIGHT_CB is the callback to call when entering/leaving
   menu items. If POP_UP_P, create a popup menu. If MENU_BAR_P, create
   a menu bar. TOPMENU is the topmost GtkWidget that others shall be
   placed under. It may be NULL, in that case we create the
   appropriate widget (menu bar or menu item depending on POP_UP_P and
   MENU_BAR_P) CL_DATA is the callback data we shall use for this
   menu, or NULL if we haven't set the first callback yet. NAME is the
   name to give to the top level menu if this function creates it. May
   be NULL to not set any name.
...
static GtkWidget *
create_menus (widget_value *data, struct frame *f,
	      GCallback select_cb, GCallback deactivate_cb,
	      GCallback highlight_cb, bool pop_up_p, bool menu_bar_p,
	      GtkWidget *topmenu, xg_menu_cb_data *cl_data,
	      const char *name)
{
  widget_value *item;
  ...
  if (!topmenu)
      ...
      if (!menu_bar_p)
	{
	  ...
	  /* Connect this to the menu instead of items so we get
	     enter/leave for disabled items also.  TODO:  Still does
	     not get enter/leave for disabled items in detached menus.
	   */
	  g_signal_connect (G_OBJECT (wmenu), "enter-notify-event",
			    G_CALLBACK (menuitem_highlight_callback),
			    NULL);
	  g_signal_connect (G_OBJECT (wmenu), "leave-notify-event",
			    G_CALLBACK (menuitem_highlight_callback),
			    NULL);
	}
      else
...
# ifdef HAVE_PGTK
	  g_signal_connect (G_OBJECT (wmenu), "button-press-event",
			    ...
			    f);
# endif
	  /* Set width of menu bar to a small value so it doesn't
	     enlarge a small initial frame size.  The width will be
	     set to the width of the frame later on when it is added
	     to a container. height -1: Natural height.  */
	  gtk_widget_set_size_request (wmenu, 1, -1);
	}

      /* Put cl_data on the top menu for easier access.  */
      cl_data = make_cl_data (cl_data, f, highlight_cb);
      g_object_set_data (G_OBJECT (wmenu), XG_FRAME_DATA,
			 (gpointer) cl_data);
...
# ifndef HAVE_PGTK
      if (deactivate_cb)
	g_signal_connect (G_OBJECT (wmenu), "selection-done",
			  deactivate_cb, 0);
# else
      if (deactivate_cb)
	g_signal_connect (G_OBJECT (wmenu), "deactivate",
			  deactivate_cb, 0);
# endif
    }
  ...
  for (item = data; item; item = item->next)
	...
	{
	  w = xg_create_one_menuitem (item, f,
				      item->contents ? 0 : select_cb,
				      highlight_cb, cl_data, &group);

	  /* Create a possibly empty submenu for menu bar items, since
	     some themes don't highlight items correctly without it.
	   */
	  if (item->contents || menu_bar_p)
	    {
	      GtkWidget *submenu
		= create_menus (item->contents, f, select_cb,
				deactivate_cb, highlight_cb, 0, 0, 0,
				cl_data, 0);
...
/* Create a menubar, popup menu or dialog, depending on the TYPE
   argument. TYPE can be "menubar", "popup" for popup menu, or
   "dialog" for a dialog with some text and buttons. F is the frame
   the created item belongs to. NAME is the name to use for the top
   widget. VAL is a widget_value structure describing items to be
   created. SELECT_CB is the callback to use when a menu item is
   selected or a dialog button is pressed. DEACTIVATE_CB is the
   callback to use when an item is deactivated. For a menu, when a sub
   menu is not shown anymore, for a dialog it is called when the
   dialog is popped down. HIGHLIGHT_CB is the callback to call when
   entering/leaving menu items.
		  ...
		  GCallback deactivate_cb, GCallback highlight_cb)
{
  ...
  if (strcmp (type, "dialog") == 0)
    ...
    {
      w = create_menus (val->contents, f, select_cb, deactivate_cb,
			highlight_cb, pop_up_p, menu_bar_p, 0, 0,
			name);

      /* Set the cursor to an arrow for popup menus when they are
	 mapped. This is done by default for menu bar menus.  */
      if (pop_up_p)
	{
	  /* Must realize so the GdkWindow inside the widget is
	   * created.  */
	  gtk_widget_realize (w);
  ...
  for (iter = list; iter; iter = g_list_next (iter))
    {
      ...
      /* Destroying the widget will remove it from the container it is
       * in.  */
      gtk_widget_destroy (w);
    }
}
...
/* Update the top level names in MENUBAR (i.e. not submenus).
   ...
   *LIST is a list with the current menu bar names (menu item
   widgets). ITER is the item within *LIST that shall be updated. POS
   is the numerical position, starting at 0, of ITER in *LIST. VAL
   describes what the menu bar shall look like after the update.
   SELECT_CB is the callback to use when a menu item is selected.
   HIGHLIGHT_CB is the callback to call when entering/leaving menu
   items. CL_DATA points to the callback data to be used for this menu
   bar.

   This function calls itself to walk through the menu bar names.  */

static void
xg_update_menubar (GtkWidget *menubar, struct frame *f, GList **list,
		   GList *iter, int pos, widget_value *val,
		   GCallback select_cb, GCallback deactivate_cb,
		   GCallback highlight_cb, xg_menu_cb_data *cl_data)
{
  if (!iter && !val)
    return;
  else if (iter && !val)
    {
      ...
      gtk_menu_shell_insert (GTK_MENU_SHELL (menubar),
			     gtk_menu_item_new_with_label (""), 0);
      /* All updated.  */
      val = 0;
      iter = 0;
    }
  else if (!iter && val)
    {
      ...
      create_menus (val, f, select_cb, deactivate_cb, highlight_cb, 0,
		    1, menubar, cl_data, 0);
  ...
  /* Below this neither iter or val is NULL */
  else if (xg_item_label_same_p (GTK_MENU_ITEM (iter->data),
				 val->name))
    {
  ...
  else /* This item is changed.  */
    {
      ...
      /* See if the changed entry (val) is present later in the menu
       * bar  */
      for (iter2 = iter; iter2 && !val_in_menubar;
      ...
      else if (!val_in_menubar && !iter_in_new_menubar)
	{
	  /*  This corresponds to:
		Current:  A B C
		New:      A X C
	      Rename B to X.  This might seem to be a strange thing to
	     do, since if there is a menu under B it will be totally
	     wrong for X. But consider editing a C file.  Then there
	     is a C-mode menu (corresponds to B above). If then doing
	     C-x C-f the minibuf menu (X above) replaces the C-mode
	     menu.  When returning from the minibuffer, we get back
	     the C-mode menu.  Thus we do: Rename B to X (C-mode to
	     minibuf menu) Rename X to B (minibuf to C-mode menu). If
	     the X menu hasn't been invoked, the menu under B is up to
	     date when leaving the minibuffer.  */
	  GtkLabel *wlabel = GTK_LABEL (XG_BIN_CHILD (witem));
      ...
      else if (!val_in_menubar && iter_in_new_menubar)
	{
	  ...
	  GSList *group = 0;
	  GtkWidget *w
	    = xg_create_one_menuitem (val, f, select_cb, highlight_cb,
				      cl_data, &group);
	   ...
	   */
	  GtkWidget *submenu
	    = create_menus (NULL, f, select_cb, deactivate_cb,
			    highlight_cb, 0, 0, 0, cl_data, 0);
  ...
  xg_update_menubar (menubar, f, list, iter, pos, val, select_cb,
...
/* Update the menu item W so it corresponds to VAL.
   SELECT_CB is the callback to use when a menu item is selected.
   HIGHLIGHT_CB is the callback to call when entering/leaving menu
   items. CL_DATA is the data to set in the widget for menu
   invocation.  */

static void
xg_update_menu_item (widget_value *val, GtkWidget *w,
		     GCallback select_cb, GCallback highlight_cb,
		     xg_menu_cb_data *cl_data)
{
  ...
  if (wkey)
    old_key = gtk_label_get_label (wkey);
  if (wlbl)
    old_label = gtk_label_get_label (wlbl);

  if (wkey && utf8_key
      && (!old_key || strcmp (utf8_key, old_key) != 0))
    {
      label_changed = true;
...
/* Update the sub menu SUBMENU and all its children so it corresponds
   to VAL. SUBMENU may be NULL, in that case a new menu is created. F
   is the frame the menu bar belongs to. VAL describes the contents of
   the menu bar. SELECT_CB is the callback to use when a menu item is
   selected. DEACTIVATE_CB is the callback to use when a sub menu is
   not shown anymore. HIGHLIGHT_CB is the callback to call when
   entering/leaving menu items. CL_DATA is the call back data to use
   for any newly created items.

   Returns the updated submenu widget, that is SUBMENU unless SUBMENU
   was NULL.  */

static GtkWidget *
xg_update_submenu (GtkWidget *submenu, struct frame *f,
		   widget_value *val, GCallback select_cb,
		   GCallback deactivate_cb, GCallback highlight_cb,
		   xg_menu_cb_data *cl_data)
{
  ...
  for (cur = val, iter = list; cur && iter;
       iter = g_list_next (iter), cur = cur->next)
    {
      GtkWidget *w = GTK_WIDGET (iter->data);

      /* Remember first radio button in a group.  If we get a mismatch
	 in a radio group we must rebuild the whole group so that the
	 connections in GTK becomes correct.  */
      ...
      else if (GTK_IS_CHECK_MENU_ITEM (w))
	{
	  ...
	  xg_update_menu_item (cur, w, select_cb, highlight_cb,
			       cl_data);
	}
	...
	{
	  /* Structural difference.  Remove everything from here and
	     down in SUBMENU.  */
	  break;
	}
    }
  ...
  if (cur)
    {
      ...
      newsub = create_menus (cur, f, select_cb, deactivate_cb,
			     highlight_cb, 0, 0, submenu, cl_data, 0);
    }
...
/* Update the MENUBAR.
   ...
   the MENUBAR.  If DEEP_P is zero, just rebuild the names in the
   menubar. SELECT_CB is the callback to use when a menu item is
   selected. DEACTIVATE_CB is the callback to use when a sub menu is
   not shown anymore. HIGHLIGHT_CB is the callback to call when
   entering/leaving menu items.  */
  ...
  if (deep_p)
    {
	 ...
	 X Window in the XEvent that activates the menu are those
	 widgets.  */
      ...
      for (cur = val->contents; cur; cur = cur->next)
	{
	  ...
	  newsub = xg_update_submenu (sub, f, cur->contents,
				      select_cb, deactivate_cb,
				      highlight_cb, cl_data);
	  /* sub may still be NULL.  If we just updated non deep and
	     added a new menu bar item, it has no sub menu yet.  So we
	     set the newly created sub menu under witem.  */
...
xg_update_frame_menubar (struct frame *f)
{
  ...
  if (x->menubar_widget && gtk_widget_get_parent (x->menubar_widget))
    return; /* Already done this, happens for frames created
	       invisible.  */
  ...
  gtk_box_pack_start (GTK_BOX (x->vbox_widget), x->menubar_widget,
		      FALSE, FALSE, 0);
  gtk_box_reorder_child (GTK_BOX (x->vbox_widget), x->menubar_widget,
			 0);

  g_signal_connect (x->menubar_widget, "map",
		    G_CALLBACK (menubar_map_cb), f);
...
/* Get rid of the menu bar of frame F, and free its storage.
   This is used when deleting a frame, and when turning off the menu
   bar.  */
  ...
  if (x->menubar_widget)
    {
      ...
      gtk_container_remove (GTK_CONTAINER (x->vbox_widget),
			    x->menubar_widget);
      /* The menubar and its children shall be deleted when removed
	 from the container.  */
      x->menubar_widget = 0;
...
#  ifdef HAVE_XINPUT2
  ...
  if (event->type
      == GenericEvent) /* XI_ButtonPress or XI_ButtonRelease or a
			  touch event.  */
    {
      if (!(xev->event_x >= 0 && xev->event_x < FRAME_PIXEL_WIDTH (f)
	    && xev->event_y >= 0
...
#  endif
    gw = gdk_x11_window_lookup_for_display (gdpy,
  ...
  if (!gwdesc)
    return 0;
  if (!GTK_IS_MENU_BAR (gwdesc) && !GTK_IS_MENU_ITEM (gwdesc)
      && !gtk_widget_is_ancestor (x->menubar_widget, gwdesc))
    return 0;

  list
    = gtk_container_get_children (GTK_CONTAINER (x->menubar_widget));
  if (!list)
    return 0;
  int scale = xg_get_scale (f);
#  ifdef HAVE_XINPUT2
  ...
  for (iter = list; iter; iter = g_list_next (iter))
    {
	  ...
	  && gtk_widget_intersect (w, &rec, NULL))
	break;
    }
  g_list_free (list);
  return iter != 0;
...
/* Xlib's `Window' fits in 32 bits.  But we want to store pointers,
   and they may be larger than 32 bits.  Keep a mapping from integer
   index to widget pointers to get around the 32 bit limitation.  */
...
/* Store the widget pointer W in id_to_widget and return the integer
 * index.  */
...
xg_store_widget_in_map (GtkWidget *w)
{
  ...
  if (id_to_widget.max_size == id_to_widget.used)
    {
      ptrdiff_t new_size;
      if (TYPE_MAXIMUM (Window) - ID_TO_WIDGET_INCR
	  < id_to_widget.max_size)
	memory_full (SIZE_MAX);

      new_size = id_to_widget.max_size + ID_TO_WIDGET_INCR;
      id_to_widget.widgets
	= xnrealloc (id_to_widget.widgets, new_size,
		     sizeof (GtkWidget *));
...
# endif
  ...
  vadj = gtk_adjustment_new (XG_SB_MIN, XG_SB_MIN, XG_SB_MAX, 0.1,
			     0.1, 0.1);
  wscroll = gtk_scrollbar_new (GTK_ORIENTATION_VERTICAL,
			       GTK_ADJUSTMENT (vadj));
  g_object_ref_sink (G_OBJECT (wscroll));
  gtk_widget_style_get (wscroll, "slider-width", &w, "trough-border",
			&b, NULL);
...
# endif
  ...
  hadj = gtk_adjustment_new (YG_SB_MIN, YG_SB_MIN, YG_SB_MAX, 0.1,
			     0.1, 0.1);
...
/* Callback invoked when scroll bar WIDGET is destroyed.
   ...
   We free pointer to last scroll bar values here and remove the
   index.  */
...
static void
xg_finish_scroll_bar_creation (struct frame *f, GtkWidget *wscroll,
...
# ifndef HAVE_GTK3
  gtk_range_set_update_policy (GTK_RANGE (wscroll),
...
# endif
  ...
  g_signal_connect (G_OBJECT (wscroll), "change-value",
		    scroll_callback, (gpointer) bar);
  g_signal_connect (G_OBJECT (wscroll), "button-release-event",
		    end_callback, (gpointer) bar);

  /* The scroll bar widget does not draw on a window of its own.
     Instead it draws on the parent window, in this case the edit
     widget.  So whenever the edit widget is cleared, the scroll bar
     needs to redraw also, which causes flicker.  Put an event box
     between the edit widget and the scroll bar, so the scroll bar
     instead draws itself on the event box window.  */
  gtk_fixed_put (GTK_FIXED (f->output_data.xp->edit_widget), webox,
		 -1, -1);
...
void
xg_create_scroll_bar (struct frame *f, struct scroll_bar *bar,
		      GCallback scroll_callback,
		      GCallback end_callback,
		      const char *scroll_bar_name)
{
...
/* Create a horizontal scroll bar widget for frame F.  Store the
   scroll bar in BAR.  SCROLL_CALLBACK is the callback to invoke when
   the value of the bar changes.  END_CALLBACK is the callback to
   invoke when scrolling ends.  SCROLL_BAR_NAME is the name we use for
   the scroll bar.  Can be used to set resources for the widget.  */
...
/* Update the position of the vertical scroll bar represented by
   SCROLLBAR_ID in frame F. TOP/LEFT are the new pixel positions where
   the bar shall appear. WIDTH, HEIGHT is the size in pixels the bar
   shall have.  */

void
xg_update_scrollbar_pos (struct frame *f, ptrdiff_t scrollbar_id,
...
# endif
      ...
      if (gtk_widget_get_parent (wparent) == wfixed)
	{
      ...
      if (hidden)
	{
	  /* No room.  Hide scroll bar as some themes output a warning
	     if the height is less than the min size.  */
...
# if !defined HAVE_PGTK && defined HAVE_GTK3
	  if (wdesc)
	    {
	      gdk_window_move_resize (wdesc, left, top, width,
				      height);
...
# endif
	...
	/* GTK does not redraw until the main loop is entered again,
	   but if there are no X events pending we will not enter it.
	   So we sync here to get some events.  */
...
/* Update the position of the horizontal scroll bar represented by
   SCROLLBAR_ID in frame F. TOP/LEFT are the new pixel positions where
   the bar shall appear. WIDTH, HEIGHT is the size in pixels the bar
   shall have.  */
...
xg_update_horizontal_scrollbar_pos (struct frame *f,
				    ptrdiff_t scrollbar_id, int top,
...
# endif
	...
	{
	  /* No room.  Hide scroll bar as some themes output a warning
	     if the width is less than the min size.  */
	  gtk_widget_hide (wparent);
	  gtk_widget_hide (wscroll);
	}
      else
...
# endif
	 ...
	 if there are no X events pending we will not enter it.  So we
	 sync here to get some events.  */
...
void
xg_set_toolkit_scroll_bar_thumb (struct scroll_bar *bar, int portion,
				 int position, int whole)
{
  struct frame *f = XFRAME (WINDOW_FRAME (XWINDOW (bar->window)));
  GtkWidget *wscroll
    = xg_get_widget_from_map (bar->x_window, FRAME_X_DISPLAY (f));
  ...
  if (wscroll && bar->dragging == -1)
      ...
      if (scroll_bar_adjust_thumb_portion_p)
	{
	  ...
	  /* When the thumb is at the bottom, position == whole.
	     So we need to increase `whole' to make space for the
	     thumb.  */
	  whole += portion;
	}
      ...
      else
	{
      ...
      value = clip_to_bounds (XG_SB_MIN, top * XG_SB_RANGE,
			      XG_SB_MAX - size);
      ...
      if (old_size != size)
	{
      ...
      if (changed
	  || int_gtk_range_get_value (GTK_RANGE (wscroll)) != value)
	{
	  block_input ();
	  ...
	  if (int_gtk_range_get_value (GTK_RANGE (wscroll)) != value)
	    gtk_range_set_value (GTK_RANGE (wscroll),
				 (gdouble) value);
...
# endif /* HAVE_XINPUT2 */
    {
      /* If we are releasing or moving the scroll bar, it has the
       * grab.  */
...
#  define GETSETUP(f) make_float (f (page_setup, GTK_UNIT_POINTS))
  return list (Fcons (Qorientation, orientation_symbol),
	       Fcons (Qwidth,
		      GETSETUP (gtk_page_setup_get_page_width)),
	       Fcons (Qheight,
		      GETSETUP (gtk_page_setup_get_page_height)),
	       Fcons (Qleft_margin,
		      GETSETUP (gtk_page_setup_get_left_margin)),
	       Fcons (Qright_margin,
		      GETSETUP (gtk_page_setup_get_right_margin)),
	       Fcons (Qtop_margin,
		      GETSETUP (gtk_page_setup_get_top_margin)),
...
xg_print_frames_dialog (Lisp_Object frames)
{
  ...
  g_signal_connect (print, "draw-page", G_CALLBACK (draw_page),
		    &frames);
  res = gtk_print_operation_run (
    print, GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG, NULL, NULL);
...
/* The key for storing the latest modifiers so the activate callback
   can get them.  */
# define XG_TOOL_BAR_LAST_MODIFIER "emacs-tool-bar-modifier"

/* The key for the data we put in the GtkImage widgets.  The data is
   the stock name used by Emacs.  We use this to see if we need to
   update the GtkImage with a new image.  */
...
static gboolean
xg_tool_bar_button_cb (GtkWidget *widget, GdkEventButton *event,
		       gpointer user_data)
{
  intptr_t state = event->state;
  gpointer ptr = (gpointer) state;
  g_object_set_data (G_OBJECT (widget), XG_TOOL_BAR_LAST_MODIFIER,
		     ptr);
  return FALSE;
}
...
xg_tool_bar_callback (GtkWidget *w, gpointer client_data)
{
  ...
  gpointer gmod
    = g_object_get_data (G_OBJECT (w), XG_TOOL_BAR_LAST_MODIFIER);
  ...
  /* Convert between the modifier bits GDK uses and the modifier bits
     Emacs uses.  This assumes GDK and X masks are the same, which
     they are when this is written.  */
# ifndef HAVE_PGTK
  event.modifiers
    = x_x_to_emacs_modifiers (FRAME_DISPLAY_INFO (f), mod);
# else
  event.modifiers
    = pgtk_gtk_to_emacs_modifiers (FRAME_DISPLAY_INFO (f), mod);
# endif
  kbd_buffer_store_event (&event);
...
/* This callback is called when the mouse enters or leaves a tool bar
   item. It is used for displaying and hiding the help text. W is the
   tool bar item, a button. EVENT is either an enter event or leave
   event. CLIENT_DATA is an integer that is the index of the button in
   the tool bar.  0 is the first button.

   Returns FALSE to tell GTK to keep processing this event.  */

static gboolean
xg_tool_bar_help_callback (GtkWidget *w, GdkEventCrossing *event,
			   gpointer client_data)
{
  intptr_t idx = (intptr_t) client_data;
  struct frame *f = g_object_get_data (G_OBJECT (w), XG_FRAME_DATA);
...
/* This callback is called when a tool bar item shall be redrawn.
   It modifies the expose event so that the GtkImage widget redraws
   the whole image.  This to overcome a bug that makes GtkImage draw
   the image in the wrong place when it tries to redraw just a part of
   the image. W is the GtkImage to be redrawn. EVENT is the expose
   event for W. CLIENT_DATA is unused.

   Returns FALSE to tell GTK to keep processing this event.  */

# ifndef HAVE_GTK3
static gboolean
xg_tool_bar_item_expose_callback (GtkWidget *w, GdkEventExpose *event,
				  gpointer client_data)
{
  gint width, height;
  ...
  event->area.x
    -= width > event->area.width ? width - event->area.width : 0;
  event->area.y
    -= height > event->area.height ? height - event->area.height : 0;
...
xg_pack_tool_bar (struct frame *f, Lisp_Object pos)
{
  ...
  if (into_hbox)
    {
      gtk_box_pack_start (GTK_BOX (x->hbox_widget), top_widget, FALSE,
			  FALSE, 0);

      if (EQ (pos, Qleft))
	gtk_box_reorder_child (GTK_BOX (x->hbox_widget), top_widget,
			       0);
  ...
  else
    {
      ...
      gtk_box_pack_start (GTK_BOX (x->vbox_widget), top_widget, FALSE,
			  FALSE, 0);

      if (EQ (pos, Qtop))
	gtk_box_reorder_child (GTK_BOX (x->vbox_widget), top_widget,
			       vbox_pos);
...
static void
tb_size_cb (GtkWidget *widget, GdkRectangle *allocation,
	    gpointer user_data)
{
  /* When tool bar is created it has one preferred size.  But when
     size is allocated between widgets, it may get another.  So we
     must update size hints if tool bar size changes.  Seen on Fedora
     18 at least.  */
  struct frame *f = user_data;
...
# endif
  ...
  gtk_toolbar_set_style (GTK_TOOLBAR (x->toolbar_widget),
			 GTK_TOOLBAR_ICONS);
  gtk_orientable_set_orientation (GTK_ORIENTABLE (x->toolbar_widget),
				  GTK_ORIENTATION_HORIZONTAL);
...
/* Find the right-to-left image named by RTL in the tool bar images
   for F. Returns IMAGE if RTL is not found.  */
...
static GtkToolItem *
xg_make_tool_item (struct frame *f, GtkWidget *wimage,
		   GtkWidget **wbutton, const char *label, int i,
		   bool horiz, bool text_image)
{
  GtkToolItem *ti = gtk_tool_item_new ();
  GtkWidget *vb = gtk_box_new (horiz ? GTK_ORIENTATION_HORIZONTAL
				     : GTK_ORIENTATION_VERTICAL,
			       0);
...
# ifdef HAVE_GTK3
  ...
  gtk_style_context_add_provider (gsty, GTK_STYLE_PROVIDER (css_prov),
				  GTK_STYLE_PROVIDER_PRIORITY_USER);
...
# endif
  ...
  if (label)
    gtk_box_pack_start (GTK_BOX (vb), gtk_label_new (label), TRUE,
			TRUE, 0);
  if (wimage && text_image)
    gtk_box_pack_start (GTK_BOX (vb), wimage, TRUE, TRUE, 0);
  ...
  if (wimage || label)
    {
      ...
      g_signal_connect (G_OBJECT (wb), "clicked",
			G_CALLBACK (xg_tool_bar_callback), gi);
...
# ifndef HAVE_GTK3
	 ...
	 comment for xg_tool_bar_item_expose_callback.  */
      g_signal_connect (G_OBJECT (ti), "expose-event",
			G_CALLBACK (xg_tool_bar_item_expose_callback),
			0);
# endif
      gtk_tool_item_set_homogeneous (ti, FALSE);

      /* Callback to save modifier mask (Shift/Control, etc).  GTK
	 makes no distinction based on modifiers in the activate
	 callback, so we have to do it ourselves.  */
	 ...
	 will tell us what kind of event it is.  */
      g_signal_connect (G_OBJECT (weventbox), "enter-notify-event",
			G_CALLBACK (xg_tool_bar_help_callback), gi);
      g_signal_connect (G_OBJECT (weventbox), "leave-notify-event",
			G_CALLBACK (xg_tool_bar_help_callback), gi);
    }
...
xg_update_tool_bar_sizes (struct frame *f)
{
  ...
  if (nl != FRAME_TOOLBAR_LEFT_WIDTH (f)
	...
	= FRAME_TOOLBAR_BOTTOM_HEIGHT (f) = 0;
...
static char *
find_icon_from_name (char *name, GtkIconTheme *icon_theme,
		     char **icon_name)
{
# ifndef HAVE_GTK3
...
# endif
  ...
  if (!NILP (tbinfo->last_tool_bar) && !NILP (f->tool_bar_items)
      ...
      && tbinfo->dir == dir && !NILP (Fequal (tbinfo->style, style))
  ...
  for (i = j = 0; i < f->n_tool_bar_items; ++i)
    {
      ...
      Lisp_Object label = (EQ (style, Qimage) || (vert_only && horiz))
			    ? Qnil
	  ...
	  if (VECTORP (image))
	    {
	      if (enabled_p)
		idx
		  = (selected_p ? TOOL_BAR_IMAGE_ENABLED_SELECTED
				: TOOL_BAR_IMAGE_ENABLED_DESELECTED);
	      else
		idx
		  = (selected_p ? TOOL_BAR_IMAGE_DISABLED_SELECTED
...
# endif
	 ...
	 remove it and make a new tool item from scratch.  */
      if (ti
	  && xg_tool_item_stale_p (wbutton, stock_name, icon_name,
				   img,
				   NILP (label)	     ? NULL
...
# if HAVE_XFT
  /* Work around a bug with corrupted data if libXft gets unloaded.
     This way we keep it permanently linked in.  */
  XftInit (0);
# endif
...
# ifndef HAVE_GTK3
  /* Remove F10 as a menu accelerator, it does not mix well with Emacs
     key bindings.  It doesn't seem to be any way to remove
     properties, so we set it to "" which in means "no key".  */
...
# else
  gtk_settings_set_string_property (settings, "gtk-key-theme-name",
				    "Emacs", EMACS_CLASS);
# endif
...
#  endif
  ...
  ie.arg = decode_string_utf_8 (Qnil, str, strlen (str), Qnil, false,
				Qnil, Qnil);
  ...
  Fput_text_property (make_fixnum (0), make_fixnum (SCHARS (ie.arg)),
		      Qcoding, Qt, ie.arg);
...
#  endif
  ...
  xstate
    = xg_virtual_mods_to_x (FRAME_DISPLAY_INFO (f), event->key.state);
...
#  endif
  ...
  if (uc)
    {
      inev.ie.kind
	= (SINGLE_BYTE_CHAR_P (uc) ? ASCII_KEYSTROKE_EVENT
...
{
  GdkEvent *xg_event
    = gdk_event_new ((xkey->type == KeyPress
#  ifdef HAVE_XINPUT2
...
#  if GTK_CHECK_VERSION(3, 20, 0)
  ...
  gdk_event_set_device (xg_event, gdk_seat_get_keyboard (seat));
#  elif GTK_CHECK_VERSION(3, 16, 0)
  GdkDeviceManager *manager = gdk_display_get_device_manager (dpy);
  GList *devices
    = gdk_device_manager_list_devices (manager,
...
#  endif
      ...
      gdk_keymap_translate_keyboard_state (keymap, xkey->xkey.keycode,
					   xkey->xkey.state,
					   xg_event->key.group,
					   &xg_event->key.keyval,
					   NULL, NULL, &consumed);
...
#  if GTK_CHECK_VERSION(3, 6, 0)
      xg_event->key.is_modifier
	= gdk_x11_keymap_key_is_modifier (keymap,
					  xg_event->key
					    .hardware_keycode);
#  endif
#  ifdef HAVE_XINPUT2
  ...
  else
    {
      ...
      gdk_keymap_translate_keyboard_state (keymap, xev->detail,
					   xev->mods.effective,
					   xg_event->key.group,
					   &xg_event->key.keyval,
					   NULL, NULL, &consumed);
...
#  endif
  ...
  result = gtk_im_context_filter_keypress (FRAME_X_OUTPUT (frame)
					     ->im_context,
					   &xg_event->key);
