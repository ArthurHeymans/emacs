	  ...
	  if (t1->data)
	    {
	      ...
	      rec->name
		= make_formatted_string ("%u:%s", source, name);
	    }
	  else
	    {
	      /* GTK bug 7737 results in GDK seats being initialized
		 with NULL devices in some circumstances.  As events
...
#ifdef USE_SKIA
# ifdef SK_GL
  /* For GL surfaces, use direct GPU blit which avoids creating an
     SkImage snapshot.  This reduces allocation overhead and keeps
     the operation entirely on the GPU.

     Note: GL has Y=0 at bottom, while window coords have Y=0 at top.
     We must flip Y coordinates when using GL functions.  */
  if (FRAME_GL_FRAMEBUFFER (f) && FRAME_GL_TEXTURE (f)
      && FRAME_SKIA_SURFACE (f))
    {
      int surface_height
	= emacs_skia_surface_get_height (FRAME_SKIA_SURFACE (f));

      int src_x = (int) src_rect->x;
      int src_y = (int) src_rect->y;
      int width = (int) src_rect->width;
      int height = (int) src_rect->height;
      int dst_x = (int) dst_rect->x;
      int dst_y = (int) dst_rect->y;

      /* Convert window Y coordinates to GL Y coordinates (flip).  */
      int gl_src_y = surface_height - src_y - height;
      int gl_dst_y = surface_height - dst_y - height;

      /* Flush any pending Skia operations before direct GL access.  */
	...
	gdk_gl_context_make_current (FRAME_GDK_GL_CONTEXT (f));
      ...
      /* Create a staging texture and FBO for the copy.  This handles
	 overlapping regions correctly.  */
      GLuint staging_tex, staging_fbo;
      glGenTextures (1, &staging_tex);
      glGenFramebuffers (1, &staging_fbo);

      /* Set up staging texture with the source region size.  */
      ...
      /* Copy source region from main FBO to staging texture.
	 glCopyTexSubImage2D reads from current READ_FRAMEBUFFER.  */
      glBindFramebuffer (GL_READ_FRAMEBUFFER, FRAME_GL_FRAMEBUFFER (f));
      glCopyTexSubImage2D (GL_TEXTURE_2D, 0, 0, 0,
			   src_x, gl_src_y, width, height);

      /* Attach staging texture to staging FBO for reading.  */
      glBindFramebuffer (GL_FRAMEBUFFER, staging_fbo);
      glFramebufferTexture2D (GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
			      GL_TEXTURE_2D, staging_tex, 0);

      /* Blit from staging FBO to main FBO at destination.
	 glBlitFramebuffer preserves orientation when src and dst
	 have same winding, so we use (0,0)-(w,h) for both.  */
      glBindFramebuffer (GL_READ_FRAMEBUFFER, staging_fbo);
      glBindFramebuffer (GL_DRAW_FRAMEBUFFER, FRAME_GL_FRAMEBUFFER (f));
      glBlitFramebuffer (0, 0, width, height,
			 dst_x, gl_dst_y, dst_x + width, gl_dst_y + height,
			 GL_COLOR_BUFFER_BIT, GL_NEAREST);

      /* Clean up staging resources.  */
      glDeleteFramebuffers (1, &staging_fbo);
...
# endif /* SK_GL */
  ...
  /* Fallback: use Skia image snapshot (works for both raster and GL).  */
...
pgtk_frame_up_to_date (struct frame *f)
{
...
#if defined(USE_SKIA) && defined(SK_GL)
  ...
  if (!buffer_flipping_blocked_p ())
    {
#ifdef USE_SKIA
      /* For Skia raster (no GL), queue a regular draw.  */
      if (FRAME_SKIA_SURFACE (f))
	{
	  if (FRAME_GL_AREA (f))
	    gtk_gl_area_queue_render (GTK_GL_AREA (FRAME_GL_AREA (f)));
...
pgtk_buffer_flipping_unblocked_hook (struct frame *f)
{
  block_input ();
#ifdef USE_SKIA
  /* For Skia (both GL and raster), queue a draw on the widget.
     The GtkGLArea render callback will blit from the GL FBO.  */
  if (FRAME_SKIA_SURFACE (f))
    {
      /* Flush Skia first.  */
      emacs_skia_surface_flush (FRAME_SKIA_SURFACE (f));
...
pgtk_handle_draw (GtkWidget *widget, cairo_t *cr, gpointer *data)
{
...
#ifdef USE_SKIA
  /* This function is used for tooltips and as a fallback for raster
     surfaces.  For GL rendering, GtkGLArea handles display.  */
  ...
  /* If no surface yet, just clear and return.  */
  if (!skia_surface)
    {
      unsigned long bg = FRAME_X_OUTPUT (f)->background_color;
      cairo_set_source_rgb (cr, RED_FROM_ULONG (bg) / 255.0,
			    GREEN_FROM_ULONG (bg) / 255.0,
			    BLUE_FROM_ULONG (bg) / 255.0);
      cairo_paint (cr);
      return FALSE;
    }
  ...
  /* For raster surfaces, blit pixels directly.  */
  void *pixels = emacs_skia_surface_get_pixels (skia_surface);
  if (pixels)
    {
      int width = emacs_skia_surface_get_width (skia_surface);
      int height = emacs_skia_surface_get_height (skia_surface);
      cairo_surface_t *skia_cairo_surface
	= cairo_image_surface_create_for_data (pixels, CAIRO_FORMAT_ARGB32,
					       width, height, width * 4);
      cairo_set_source_surface (cr, skia_cairo_surface, 0, 0);
      cairo_paint (cr);
      cairo_surface_destroy (skia_cairo_surface);
    }
#else /* USE_CAIRO */
...
#else
  /* For non-GL rendering (Cairo or Skia raster), use the draw callback.  */
  g_signal_connect (G_OBJECT (FRAME_GTK_WIDGET (f)), "draw",
		    G_CALLBACK (pgtk_handle_draw), NULL);
#endif
...
:/* Hash table of character codes indexed by X keysym codes.  */);
  Vpgtk_keysym_table
    = make_hash_table (&hashtest_eql, 900, Weak_None);
