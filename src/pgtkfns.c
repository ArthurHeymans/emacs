...
static struct pgtk_display_info *
  pgtk_display_info_for_name (Lisp_Object);
...
pgtk_display_info_for_name (Lisp_Object name)
{
  ...
  if (!NILP (is_wayland_display (name)))
	...
	if (!NILP (
	      is_wayland_display (XCAR (dpyinfo->name_list_element))))
	  return dpyinfo;
    }
  else
    {
...
static void
pgtk_set_foreground_color (struct frame *f, Lisp_Object arg,
			   Lisp_Object oldval)
{
...
static void
pgtk_set_background_color (struct frame *f, Lisp_Object arg,
			   Lisp_Object oldval)
{
  unsigned long bg;
...
static void
pgtk_set_alpha_background (struct frame *f, Lisp_Object arg,
			   Lisp_Object oldval)
{
...
static void
pgtk_set_border_color (struct frame *f, Lisp_Object arg,
		       Lisp_Object oldval)
{
  int pix;
...
static void
pgtk_set_cursor_color (struct frame *f, Lisp_Object arg,
		       Lisp_Object oldval)
{
...
static void
pgtk_set_title (struct frame *f, Lisp_Object name,
...
static void
pgtk_set_menu_bar_lines (struct frame *f, Lisp_Object value,
			 Lisp_Object oldval)
{
  int nlines;
...
static void
pgtk_set_tab_bar_lines (struct frame *f, Lisp_Object value,
			Lisp_Object oldval)
{
  int nlines;
...
/* Toolbar support.  */
static void
pgtk_set_tool_bar_lines (struct frame *f, Lisp_Object value,
			 Lisp_Object oldval)
{
  int nlines;
  ...
  if (FRAME_MINIBUF_ONLY_P (f))
    {
      /* Make sure implied resizing can be inhibited for
	 minibuffer-only frames too.  */
      f->tool_bar_resized = true;
...
static void
pgtk_set_icon_type (struct frame *f, Lisp_Object arg,
		    Lisp_Object oldval)
{
  bool result;
  ...
  block_input ();
  if (NILP (arg))
    result = pgtk_text_icon (f, SSDATA ((!NILP (f->icon_name)
					   ? f->icon_name
					   : f->name)));
  else
...
static void
pgtk_set_icon_name (struct frame *f, Lisp_Object arg,
		    Lisp_Object oldval)
{
  bool result;
  ...
  result
    = pgtk_text_icon (f, SSDATA ((!NILP (f->icon_name) ? f->icon_name
...
static void
pgtk_set_cursor_type (struct frame *f, Lisp_Object arg,
		      Lisp_Object oldval)
{
...
static void
pgtk_set_mouse_color (struct frame *f, Lisp_Object arg,
		      Lisp_Object oldval)
{
}
...
static void
pgtk_set_scroll_bar_foreground (struct frame *f,
...
static void
pgtk_set_scroll_bar_background (struct frame *f,
				Lisp_Object new_value,
				Lisp_Object old_value)
{
  GtkCssProvider *css_provider
  ...
  if (NILP (new_value))
	...
	error ("Unknown color");
	 ...
	 gtk theme.  (It honors the GTK theme if not explicitly set,
	 so I see no harm in letting users tinker a bit more.)  */
      char css[64];
...
(Lisp_Object monitor_model, Lisp_Object scale_factor)
{
  ...
  Lisp_Object tem
    = Fassoc (monitor_model, monitor_scale_factor_alist, Qnil);
  if (NILP (tem))
    {
      if (!NILP (scale_factor))
	monitor_scale_factor_alist
	  = Fcons (Fcons (monitor_model, scale_factor),
		   monitor_scale_factor_alist);
    }
  else
...
(Lisp_Object parms)
{
  ...
  if (!NILP (parent) || BASE_EQ (parent_frame, Qunbound)
...
# endif /* HAVE_HARFBUZZ */
  ...
  gui_default_parameter (f, parms, Qinternal_border_width,
			 make_fixnum (0), "internalBorderWidth",
...
#undef INSTALL_CURSOR
  ...
  gui_default_parameter (f, parms, Qalpha_background, Qnil,
			 ...
			 RES_TYPE_NUMBER);
  gui_default_parameter (f, parms, Qborders_respect_alpha_background,
  ...
  if (FRAME_GTK_OUTER_WIDGET (f))
    {
      GList *w = gtk_container_get_children (
  ...
  if (!FRAME_X_OUTPUT (f)->explicit_parent)
      ...
      else
	{
	 ...
	 from `x-create-frame-with-faces' (see above comment).  */
      f->was_invisible = (f->was_invisible
			  && (!BASE_EQ (height, Qunbound)
			      || !BASE_EQ (width, Qunbound)));
  ...
  /* All remaining specified parameters, which have not been "used"
     by gui_display_get_arg and friends, now go in the misc. alist of
     the frame.  */
  ...
  FRAME_X_OUTPUT (f)->scale_factor_atimer
    = start_atimer (ATIMER_CONTINUOUS, ts,
		    update_watched_scale_factor, f);
...
static void
pgtk_frame_restack (struct frame *f1, struct frame *f2,
		    bool above_flag)
{
  block_input ();
...
# define PATH_MAX_LEN                \
   (max (sizeof PATH_FOR_CLASS_TYPE, \
	 sizeof PATH_PREFIX_FOR_NAME_TYPE))
...
parse_resource_key (const char *res_key, char *setting_key)
{
   ...
   *      setting_key="cursor-blink"
   *
   * Returns GSettings* if setting_key exists in schema, otherwise
   * NULL.
   */
  ...
  *dp = '\0';
  ...
  GSettingsSchemaSource *ssrc
    = g_settings_schema_source_get_default ();
  GSettingsSchema *scm
    = g_settings_schema_source_lookup (ssrc, SCHEMA_ID, TRUE);
  if (!scm)
...
pgtk_get_defaults_value (const char *key)
{
   ...
   *
   * MEMO: Resource values for emacs shouldn't need such a long string
   * value.
   */
...
DEFUN ("x-display-screens", Fx_display_screens, Sx_display_screens, 0,
       1, 0, doc:/* SKIP: real doc in xfns.c.  */)
(Lisp_Object terminal)
{
  check_pgtk_display_info (terminal);
...
DEFUN ("x-open-connection", Fx_open_connection, Sx_open_connection, 1,
       3, 0, doc:/* SKIP: real doc in xfns.c.  */)
(Lisp_Object display, Lisp_Object resource_string,
...
DEFUN ("x-close-connection", Fx_close_connection, Sx_close_connection,
       1, 1, 0, doc:/* SKIP: real doc in xfns.c.  */)
(Lisp_Object terminal)
{
  struct pgtk_display_info *dpyinfo
    = check_pgtk_display_info (terminal);
  ...
  for (i = 0; i < n_monitors; ++i)
    {
      ...
      scale = pgtk_get_monitor_scale_factor (
...
(Lisp_Object terminal)
{
  ...
  FOR_EACH_FRAME (rest, frame)
  {
    ...
    if (FRAME_PGTK_P (f) && FRAME_DISPLAY_INFO (f) == dpyinfo
	&& !FRAME_TOOLTIP_P (f))
      {
	GdkWindow *gwin
...
pgtk_frame_scale_factor (struct frame *f)
{
  ...
  /* GTK returns scaled sizes for the workareas.  */
  double scale
    = pgtk_get_monitor_scale_factor (gdk_monitor_get_model (gmon));
  if (scale == 0.0)
...
DEFUN ("x-display-planes", Fx_display_planes, Sx_display_planes, 0, 1,
...
static Lisp_Object
pgtk_create_tip_frame (struct pgtk_display_info *dpyinfo,
		       Lisp_Object parms, struct frame *p)
{
  struct frame *f;
  Lisp_Object frame;
  Lisp_Object name;
  ...
  fset_icon_name (f, Qnil);
  FRAME_DISPLAY_INFO (f) = dpyinfo;
  f->output_data.pgtk->parent_desc
    = FRAME_DISPLAY_INFO (f)->root_window;
...
# endif /* HAVE_HARFBUZZ */
  ...
  gtk_window_set_transient_for (GTK_WINDOW (tip_window),
				GTK_WINDOW (
				  FRAME_GTK_OUTER_WIDGET (p)));
  gtk_window_set_attached_to (GTK_WINDOW (tip_window),
			      FRAME_GTK_WIDGET (p));
  ...
  f->output_data.pgtk->current_cursor
    = f->output_data.pgtk->text_cursor;
...
pgtk_hide_tip (bool delete)
{
  ...
  if ((NILP (tip_last_frame) && NILP (tip_frame))
      || (!use_system_tooltips && !delete && FRAMEP (tip_frame)
 ...
 Lisp_Object timeout, Lisp_Object dx, Lisp_Object dy)
{
	  ...
	  for (tail = parms; CONSP (tail); tail = XCDR (tail))
		  ...
		  && !EQ (parm, Qright) && !EQ (parm, Qbottom))
		{
		  ...
		  if (NILP (Fequal (Fcdr (elt), Fcdr (last))))
		    ...
		    tip_last_parms = calln (Qassq_delete_all, parm,
					    tip_last_parms);
		}
	      else
		tip_last_parms
  ...
  if (!FRAMEP (tip_frame) || !FRAME_LIVE_P (XFRAME (tip_frame)))
	...
	parms = Fcons (Fcons (Qbackground_color,
	 ...
	 variable tip_frame.  */
      if (NILP ((tip_frame
		 = pgtk_create_tip_frame (FRAME_DISPLAY_INFO (f),
					  parms, f))))
  ...
  /* Show tooltip frame.  */
  block_input ();
  gtk_window_resize (GTK_WINDOW (FRAME_GTK_OUTER_WIDGET (tip_f)),
		     width, height);
  gtk_window_move (GTK_WINDOW (FRAME_GTK_OUTER_WIDGET (tip_f)),
		   root_x, root_y);
...
DEFUN ("x-hide-tip", Fx_hide_tip, Sx_hide_tip, 0, 0, 0,
       doc:/* SKIP: real doc in xfns.c.  */)
(void) { return pgtk_hide_tip (!tooltip_reuse_hidden_frame); }
...
frame_geometry (Lisp_Object frame, Lisp_Object attribute)
{
  ...
  Lisp_Object fullscreen_symbol
    = Fframe_parameter (frame, Qfullscreen);
  ...
  if (EQ (attribute, Qouter_edges))
    ...
    return list4 (make_fixnum (native_left), make_fixnum (native_top),
		  make_fixnum (native_right),
		  make_fixnum (native_bottom));
  else if (EQ (attribute, Qinner_edges))
    return list4 (make_fixnum (native_left + internal_border_width),
		  make_fixnum (native_top + tab_bar_height
...
(void)
{
  ...
  gdk_device_get_position (device, &gscr, &x,
			   &y); /* can't get on wayland? */
...
{
#ifdef USE_SKIA
  error ("Page setup dialog not supported with Skia backend");
  return Qnil;
#else
  block_input ();
...
{
#ifdef USE_SKIA
  error ("Page setup not supported with Skia backend");
  return Qnil;
#else
  Lisp_Object result;
...
{
#ifdef USE_SKIA
  error ("Print dialog not supported with Skia backend");
  return Qnil;
#else
  Lisp_Object rest, tmp;
...
syms_of_pgtkfns (void)
{
  ...
  {
    char *ver
      = g_strdup_printf ("%d.%d.%d", GTK_MAJOR_VERSION,
			 GTK_MINOR_VERSION, GTK_MICRO_VERSION);
    int len = strlen (ver);
    Vgtk_version_string
      = make_specified_string (ver, len, len, false);
    g_free (ver);
  }
  ...
  {
    char *ver
      = g_strdup_printf ("%d.%d.%d", CAIRO_VERSION_MAJOR,
  ...
  defsubr (&Sxw_display_color_p); /* this and next called directly by
				     C code */
  ...
  DEFVAR_BOOL ("x-gtk-file-dialog-help-text",
