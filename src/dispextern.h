...
Copyright (C) 1985, 1993-1994, 1997-2026 Free Software Foundation,
Inc.
...
#ifdef HAVE_NS
# include "nsgui.h"
/* Following typedef needed to accommodate the MSDOS port, believe it
 * or not.  */
...
#define CLIP_TEXT_POS_FROM_MARKER(POS, MARKER)                 \
  (CHARPOS (POS)                                               \
   ...
   BYTEPOS (POS)                                               \
   = clip_to_bounds (BEGV_BYTE, marker_byte_position (MARKER), \
		     ZV_BYTE))
...
INLINE int
GLYPH_FACE (GLYPH glyph)
{
  return glyph.face_id;
}
...
{
  return (CONSP (gc) ? XFIXNUM (XCAR (gc)) : XFIXNUM (gc) & MAX_CHAR);
}

INLINE int
GLYPH_CODE_FACE (Lisp_Object gc)
{
  return CONSP (gc) ? XFIXNUM (XCDR (gc))
...
struct glyph
{
  /* Position from which this glyph was drawn.  If `object' below is a
     ...
     special glyphs have special values for this:
      ...
      glyph standing for newline of an empty line buffer position of
     newline stretch glyph at left edge of R2L lines     buffer
     position of newline  */
  ptrdiff_t charpos;
...
#define GLYPH_EQUAL_P(X, Y)                                        \
  ((X)->type == (Y)->type && (X)->u.val == (Y)->u.val              \
   && GLYPH_SLICE_EQUAL_P (X, Y) && (X)->face_id == (Y)->face_id   \
   && (X)->frame == (Y)->frame && (X)->padding_p == (Y)->padding_p \
...
#define GLYPH_CHAR_AND_FACE_EQUAL_P(X, Y)                 \
  ((X)->u.ch == (Y)->u.ch && (X)->face_id == (Y)->face_id \
   && (X)->frame == (Y)->frame && (X)->padding_p == (Y)->padding_p)
...
/* Glyph Matrix.
   ...
   glyph memory, which is allocated in the form of a glyph_pool
   structure. Glyph rows in such a window matrix are slices of frame
   matrix rows.
...
struct glyph_matrix
{
  ...
  /* If this structure describes a window matrix of window W,
     ...
     the value of W->pixel_top, window_height and window_width are
     width and height of W, as returned by window_box, and
     window_vscroll is the value of W->vscroll at the time the matrix
...
struct glyph_row
{
  /* Pointers to beginnings of areas.  The end of an area A is found
     at A + 1 in the vector.  The last element of the vector is the
     end of the whole row.

     Kludge alert: Even if used[TEXT_AREA] == 0,
     glyphs[TEXT_AREA][0]'s position field is used.  It is -1 if this
     row does not correspond to any text; it is some buffer position
     if the row corresponds to an empty display line that displays a
     line end.  This is what old redisplay used to do.  (Except in
     code for terminal frames, this kludge is no longer used, I
     believe. --gerd).
  ...
  /* Number of glyphs actually filled in areas.  This could have size
     LAST_AREA, but it's 1 + LAST_AREA to simplify offset
     calculations.  */
  ...
  /* True means that at least one of the left and right fringe bitmaps
     is periodic and thus depends on the y-position of the row.  */
  bool_bf fringe_bitmap_periodic_p : 1;

  /* True means that we must draw the bitmaps of this row.  */
  bool_bf redraw_fringe_bitmaps_p : 1;

  /* In a desired matrix, true means that this row must be updated. In
     a current matrix, false means that the row has been invalidated,
     i.e. the row's contents do not agree with what is visible on the
     screen.  */
  bool_bf enabled_p : 1;

  /* True means row displays a text line that is truncated on the left
     or right side.  */
  ...
  /* True means the face of the last glyph in the text area is drawn
     to the right end of the window.  This flag is used in
  ...
  /* True means this row contains glyphs that overlap each other
     because of lbearing or rbearing.  */
  bool_bf contains_overlapping_glyphs_p : 1;

  /* True means this row is as wide as the window it is displayed in,
     including scroll bars, fringes, and internal borders.  This also
  ...
  /* True in a current row means this row is overlapped by another
   * row.  */
  ...
  /* True means this line starts in the middle of a character
     consisting of more than one glyph.  Some glyphs have been put in
     the previous row, the rest are put in this row.  */
  ...
  /* True means this row width is exactly the width of the window, and
     the final newline character is hidden in the right fringe.  */
  bool_bf exact_window_width_line_p : 1;

  /* True means this row currently shows the cursor in the right
   * fringe.  */
...
#define MATRIX_FIRST_TEXT_ROW(MATRIX)                           \
  ((MATRIX)->rows->mode_line_p                                  \
     ? (((MATRIX)->rows + 1)->mode_line_p ? (MATRIX)->rows + 2  \
					  : (MATRIX)->rows + 1) \
     : (MATRIX)->rows)
...
#define MATRIX_ROW_PARTIALLY_VISIBLE_AT_TOP_P(W, ROW) \
  (MR_PARTIALLY_VISIBLE (ROW) && MR_PARTIALLY_VISIBLE_AT_TOP (W, ROW))
...
#define MATRIX_ROW_STARTS_IN_MIDDLE_OF_CHAR_P(ROW)                   \
  ((ROW)->start.dpvec_index > 0 || (ROW)->starts_in_middle_of_char_p \
...
struct glyph_string
{
  ...
  /* True means this glyph strings face has to be drawn to the right
     end of the window's drawing area.  */
  bool_bf extends_to_end_of_line_p : 1;

  /* True means the background of this string has been drawn.  */
  bool_bf background_filled_p : 1;

  /* True means that the original font determined for drawing this
     glyph string could not be loaded.  The member `font' has been set
     to the frame's default font in this case.  */
  bool_bf font_not_found_p : 1;

  /* True means that the face in which this glyph string is drawn has
     a stipple pattern.  */
...
/* Return the height of the header line in glyph matrix MATRIX, or
   zero if not known.  This macro is called under circumstances where
   MATRIX might not have been allocated yet.  */
...
#define MATRIX_TAB_LINE_HEIGHT(MATRIX)                               \
  ((MATRIX) && (MATRIX)->rows ? MATRIX_TAB_LINE_ROW (MATRIX)->height \
			      : 0)
...
#define CURRENT_MODE_LINE_ACTIVE_FACE_ID(W) \
  CURRENT_MODE_LINE_ACTIVE_FACE_ID_3 (W, XWINDOW (selected_window), W)

/* Return the current height of the mode line of window W.  If not
   known from W->mode_line_height, look at W's current glyph matrix,
   or return a default based on the height of the font of the face
   `mode-line'.  */
...
#define CURRENT_MODE_LINE_HEIGHT(W)                          \
  ((W)->mode_line_height >= 0                                \
	...
	= (MATRIX_MODE_LINE_HEIGHT ((W)->current_matrix)     \
		 ...
		 XFRAME ((W)->frame),                        \
		 CURRENT_MODE_LINE_ACTIVE_FACE_ID (W)))))

/* Return the desired face id for the header line of a window,
   depending on whether the window is selected or not, or if the
   window is the scrolling window for the currently active minibuffer
   window.
...
/* Return the current height of the header line of window W.  If not
   known from W->header_line_height, look at W's current glyph matrix,
   or return an estimation based on the height of the font of the face
   `header-line'.  */
...
/* Return the current height of the tab line of window W.  If not
   known from W->tab_line_height, look at W's current glyph matrix, or
   return an estimation based on the height of the font of the face
   `tab-line'.  */
...
/* Return proper value to be used as baseline offset of font that has
   ...
   Here, our task is to find the value of BOFF in the following
   figure;
...
#define FONT_TOO_HIGH(ft) \
  ((ft)->pixel_size > 0   \
   && (ft)->ascent + (ft)->descent > 3 * (ft)->pixel_size)

/***********************************************************************
				Faces
	***********************************************************************/
...
enum face_underline_type
{
  /* Note: order matches the order of the Smulx terminfo extension,
     and is also relied on to remain in its present order by
...
struct face
{
  ...
  /* If non-zero, this is a GC that we can use without modification
     for drawing the characters in this face.  */
# ifdef HAVE_X_WINDOWS
  GC gc;
# elif defined HAVE_ANDROID
  struct android_gc *gc;
...
# endif
  ...
  /* True means that either no color is specified for the
     corresponding attribute or that the specified color couldn't be
     loaded. Use the foreground color when drawing in that case. */
...
/* Return the id of the realized face on frame F that is like the face
   FACE, but is suitable for displaying character CHARACTER at buffer
   or string position POS.  OBJECT is the string object, or nil for
...
/* The glyph code from a display vector may either be an integer which
   encodes a char code in the lower CHARACTERBITS bits and a (very
   small) face-id in the upper bits, or it may be a cons (CHAR .
   FACE-ID).  */
...
/* Data type for describing the Bidi Paired Bracket Type of a
   character.
...
struct bidi_string_data
{
  ...
  const unsigned char
    *s;		    /* string data, or NULL if reordering buffer */
...
struct bidi_it
{
  ...
  bidi_type_t
    orig_type; /* original bidi type, as found in the buffer */
  signed char
    resolved_level; /* final resolved level of this character */
  signed char
    isolate_level; /* count of isolate initiators unmatched by PDI */
  ...
  struct bidi_saved_info
    last_strong; /* last-seen strong directional char */
  struct bidi_saved_info
    next_for_neutral; /* surrounding characters for... */
  struct bidi_saved_info prev_for_neutral; /* ...resolving neutrals */
  struct bidi_saved_info
    next_for_ws;		 /* character after sequence of ws */
  ptrdiff_t bracket_pairing_pos; /* position of pairing bracket */
  bidi_type_t bracket_enclosed_type; /* type for bracket resolution */
  ptrdiff_t
    next_en_pos; /* pos. of next char for determining ET type */
  ...
  struct bidi_stack
    level_stack[BIDI_MAXDEPTH + 2 + 1]; /* directional status stack */
  struct bidi_string_data string;	/* string to reorder */
  struct window *w;	    /* the window being displayed */
  bidi_dir_t paragraph_dir; /* current paragraph direction */
  ptrdiff_t
    separator_limit;	 /* where paragraph separator should end */
...
struct it
{
  ...
  /* Method to use to load this structure with the next display
   * element.  */
  ...
  /* Face id to use for all characters in display vector.  -1 if
   * unused. */
  int dpvec_face_id;
  ...
  /* Initial buffer or string position of the iterator, before
     skipping over display properties and invisible text.  */
  struct display_pos start;
...
#define OVERLAY_STRING_CHUNK_SIZE 16
  ...
  struct iterator_stack_entry
  {
    ...
    /* Properties from display property that are reset by another
       display property.  */
    short voffset;
  ...
  /* If true, saved_face_id contains the id of the face in front of
     text skipped due to selective display.  */
  ...
  /* First and last visible x-position in the display area.  If window
     is hscrolled by n columns, first_visible_x == n *
     FRAME_COLUMN_WIDTH (f), and last_visible_x == pixel width of W +
     first_visible_x. When truncation or continuation glyphs are
     produced due to lack of fringes, last_visible_x excludes the
     space required for these glyphs.  */
  ...
  /* Number of pixels to adjust tab stops and stretch glyphs due to
     width fixup of the first stretch glyph that crosses
     first_visible_x. This is only needed on GUI frames, only when
     display-line-numbers is in effect, and only in hscrolled windows.
   */
  int stretch_adjust;
...
typedef struct
{
  ...
  /* FRAME and X, Y position of mouse when last checked for
     highlighting.  X and Y can be negative or out of range for the
     frame.  */
...
typedef void (*frame_parm_handler) (struct frame *, Lisp_Object,
				    Lisp_Object);
...
struct redisplay_interface
{
  ...
  void (*write_glyphs) (struct window *w, struct glyph_row *row,
			struct glyph *string,
			enum glyph_row_area area, int len);
  ...
  void (*after_update_window_line_hook) (
    struct window *w, struct glyph_row *desired_row);
  ...
  void (*fix_overlapping_area) (struct window *w,
				struct glyph_row *row,
  ...
  void (*clear_frame_area) (struct frame *f, int x, int y, int width,
			    int height);
  ...
  void (*draw_window_cursor) (struct window *w,
			      struct glyph_row *glyph_row, int x,
			      int y,
  ...
  void (*draw_vertical_window_border) (struct window *w, int x,
				       int y_0, int y_1);

  /* Draw window divider for window W from (X_0, Y_0) to (X_1, ,Y_1).
   */
  void (*draw_window_divider) (struct window *w, int x_0, int x_1,
			       int y_0, int y_1);
  ...
  void (*shift_glyphs_for_insert) (struct frame *f, int x, int y,
...
# endif
  ...
  /* A single `background color' for this image, for the use of anyone
     that cares about such a thing.  Only valid if the
     `background_valid' field is true.  This should generally be
     accessed by calling the accessor macro `IMAGE_BACKGROUND', which
     will heuristically calculate a value if necessary.  */
  unsigned long background;
...
{
  /* The key of the tool-bar item.  Used to remove items when a
     binding for `undefined' is found.  */
  TOOL_BAR_ITEM_KEY,
...
extern void bidi_init_it (ptrdiff_t, ptrdiff_t, bool,
			  struct bidi_it *);
...
void window_box (struct window *, enum glyph_row_area, int *, int *,
		 int *, int *);
...
void remember_mouse_glyph (struct frame *, int, int,
			   NativeRectangle *);
...
void move_it_in_display_line (struct it *it, ptrdiff_t to_charpos,
			      int to_x, enum move_operation_enum op);
...
extern ptrdiff_t
compute_display_string_end (ptrdiff_t, struct bidi_string_data *);
...
extern void gui_get_glyph_overhangs (struct glyph *, struct frame *,
				     int *, int *);
extern struct font *
font_for_underline_metrics (struct glyph_string *);
extern void gui_produce_glyphs (struct it *);

extern void gui_write_glyphs (struct window *, struct glyph_row *,
			      struct glyph *, enum glyph_row_area,
			      int);
extern void gui_insert_glyphs (struct window *, struct glyph_row *,
			       struct glyph *, enum glyph_row_area,
			       int);
extern void gui_clear_end_of_line (struct window *,
				   struct glyph_row *,
				   enum glyph_row_area, int);
extern void gui_fix_overlapping_area (struct window *,
				      struct glyph_row *,
				      enum glyph_row_area, int);
extern void draw_phys_cursor_glyph (struct window *,
				    struct glyph_row *,
				    enum draw_glyphs_face);
extern void get_phys_cursor_geometry (struct window *,
				      struct glyph_row *,
				      struct glyph *, int *, int *,
				      int *);
extern void erase_phys_cursor (struct window *);
extern void display_and_set_cursor (struct window *, bool, int, int,
				    int, int);
...
extern int get_tab_bar_item_kbd (struct frame *, int, int, int *,
				 bool *);
extern Lisp_Object handle_tab_bar_click (struct frame *, int, int,
					 bool, int);
extern void handle_tool_bar_click (struct frame *, int, int, bool,
				   int);
extern void handle_tool_bar_click_with_device (struct frame *, int,
					       int, bool, int,
					       Lisp_Object);
...
extern void x_y_to_column_row (struct window *, int, int, int *,
			       int *);
# endif
#endif /* HAVE_WINDOW_SYSTEM */
...
#ifndef HAVE_ANDROID
extern void tty_draw_row_with_mouse_face (struct window *,
					  struct glyph_row *, int,
					  int, enum draw_glyphs_face);
...
extern ptrdiff_t image_create_bitmap_from_data (struct frame *,
						char *, unsigned int,
						unsigned int);
extern ptrdiff_t image_create_bitmap_from_file (struct frame *,
						Lisp_Object);
# if defined HAVE_XPM && defined HAVE_X_WINDOWS && !defined USE_GTK
extern ptrdiff_t x_create_bitmap_from_xpm_data (struct frame *,
						const char **);
# endif
...
int image_ascent (struct image *, struct face *,
		  struct glyph_slice *);
...
bool parse_color_spec (const char *, unsigned short *,
...
#endif
int lookup_named_face (struct window *, struct frame *, Lisp_Object,
		       bool);
int lookup_basic_face (struct window *, struct frame *, int);
int smaller_face (struct frame *, int, int);
int face_with_height (struct frame *, int, int);
int lookup_derived_face (struct window *, struct frame *, Lisp_Object,
			 int, bool);
#ifdef HAVE_WINDOW_SYSTEM
...
int face_for_overlay_string (struct window *, ptrdiff_t, ptrdiff_t *,
			     ptrdiff_t, bool, Lisp_Object,
			     enum lface_attribute_index);
int face_at_string_position (struct window *, Lisp_Object, ptrdiff_t,
			     ptrdiff_t, ptrdiff_t *, enum face_id,
			     bool, enum lface_attribute_index);
...
/* Returns the background color of IMG, calculating one heuristically
   if necessary.  If non-zero, XIMG is an existing XImage object to
   use for the heuristic.  */

# define IMAGE_BACKGROUND(img, f, ximg)         \
   ((img)->background_valid ? (img)->background \
			    : image_background (img, f, ximg))

/* Returns true if IMG has a `transparent' background, using
   heuristics to decide if necessary.  If non-zero, MASK is an
...
extern Lisp_Object buffer_posn_from_coords (struct window *, int *,
					    int *,
					    struct display_pos *,
					    Lisp_Object *, int *,
					    int *, int *, int *);
extern Lisp_Object mode_line_string (struct window *,
				     enum window_part, int *, int *,
				     ptrdiff_t *, Lisp_Object *,
				     int *, int *, int *, int *);
extern Lisp_Object marginal_area_string (struct window *,
...
void shift_glyph_matrix (struct window *, struct glyph_matrix *, int,
			 int, int);
void rotate_matrix (struct glyph_matrix *, int, int, int);
void increment_matrix_positions (struct glyph_matrix *, int, int,
				 ptrdiff_t, ptrdiff_t);
...
extern void fill_up_frame_row_with_spaces (struct frame *,
					   struct glyph_row *, int);
...
extern void set_tty_color_mode (struct tty_display_info *,
				struct frame *);
...
enum
{
  SCROLL_INFINITY = 1000000
};

/* Defined in scroll.c */

extern int scrolling_max_lines_saved (int, int, unsigned *,
				      unsigned *, int *);
extern void do_line_insertion_deletion_costs (
  struct frame *, const char *, const char *, const char *,
  const char *, const char *, const char *, int);
void scrolling_1 (struct frame *, int, int, int, int *, int *,
...
extern Lisp_Object gui_display_get_arg (Display_Info *, Lisp_Object,
					...
					enum resource_types);
extern Lisp_Object
gui_frame_get_and_record_arg (struct frame *, Lisp_Object,
